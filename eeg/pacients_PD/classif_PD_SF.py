...
for estat in range (0,2):
    ...
    res_dir = 'results/sub'+str(i_sub)+'-SF-'+type_motion[i_motion]+'-'+type_space[i_space]+ '-' + medicacio[estat] +'/'
    ...
    if i_motion == 2:
        ...
        ts_tmp[:, :, 0:108, 0] = ts_tmp2[:, :, :, 0, estat]
        ts_tmp[:, :, 108:216, 0] = ts_tmp2[:, :, :, 1, estat]
        ts_tmp[:, :, 0:108, 1] = ts_tmp2[:, :, :, 2, estat]
        ts_tmp[:, :, 108:216, 1] = ts_tmp2[:, :, :, 3, estat]
        ts_tmp[:, :, 216:324, 1] = ts_tmp2[:, :, :, 4, estat]
        ts_tmp[:, :, 324:432, 1] = ts_tmp2[:, :, :, 5, estat]
    else:
        ts_tmp = np.zeros([ts_tmp2.shape[0], ts_tmp2.shape[1], 2 * ts_tmp2.shape[2], 2])
        ts_tmp[:, :, 0:108, 0] = ts_tmp2[:, :, :, 0, estat]
        ts_tmp[:, :, 0:108, 1] = ts_tmp2[:, :, :, 1, estat]
        ts_tmp[:, :, 108:216, 1] = ts_tmp2[:, :, :, 2, estat]
