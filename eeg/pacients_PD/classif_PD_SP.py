...
i_sub = int(sys.argv[1])
# Variable per fixar si agafem els assajos amb NS, S o ALL
i_motion = int(sys.argv[2])
i_space = 1
...
for estat in range (0,2):
    #%% director to save results
    res_dir = 'results/sub'+str(i_sub)+'-SP-'+type_motion[i_motion]+'-'+type_space[i_space]+ '-' + medicacio[estat] +'/'
    ...
    if i_space == 0:
        ts_tmp2 = sio.loadmat(gen_path_dir + 'dataClean-ICA-' + str(i_sub) + '-T1.mat')['dataSorted'][:, :, :,
                  low_ind:top_ind]
    else:
        ts_tmp2 = sio.loadmat(gen_path_dir + 'dataClean-ICA-' + str(i_sub) + '-T1.mat')['ic_data'][:, :, :,
                  low_ind:top_ind]

    if i_motion == 2:
        # L'última dimensió és el nombre d'estats motivacionals
        ts_tmp = np.zeros([ts_tmp2.shape[0],ts_tmp2.shape[1],2*ts_tmp2.shape[2],2])
        ts_tmp[:, :, 0:108, 0] = ts_tmp2[:, :, :, 2, estat]
        ts_tmp[:, :, 108:216, 0] = ts_tmp2[:, :, :, 3, estat]
        ts_tmp[:, :, 0:108, 1] = ts_tmp2[:, :, :, 4, estat]
        ts_tmp[:, :, 108:216, 1] = ts_tmp2[:, :, :, 5, estat]
    else:
        ts_tmp = np.zeros([ts_tmp2.shape[0], ts_tmp2.shape[1], ts_tmp2.shape[2], 2])
        ts_tmp[:, :, 0:108, 0] = ts_tmp2[:, :, :, 1, estat]
        ts_tmp[:, :, 0:108, 1] = ts_tmp2[:, :, :, 2, estat]
    ...
    # Nombre total de canals que ens queden
    N = valid_ch.sum()
    ...
    pearson_corr_rk_shuf = np.zeros(
        [n_bands, n_measures, int(n_rep * (n_rep - 1) / 2)])  # surrogate distributions for ranking stability
    ...
    perf_rk_best_feat_pow = np.zeros(
        [n_bands, n_rep_RFE, n_feat_test_pow])  # performance when increasing number of best features (power)
    n_feat_test_FC = 80  # number of best features (corr) to test to find optimum
    perf_rk_best_feat_FC = np.zeros(
        [n_bands, n_rep_RFE, n_feat_test_FC])  # performance when increasing number of best features (corr)
    ...
    for i_band in range(n_bands):
        ...
        for i_measure in range(n_measures):
            ...
            for i_rep in range(n_rep):
                for ind_train, ind_test in cv_schem.split(vect_features, labels):  # false loop, just 1
                    ...
                    if i_measure == 0:  # power
                        ...
                        # S'obté el ranking de cada feature. És a dir, cada feature
                        # té un número associat que correspon al ranking.
                        # Es manté l'ordre de les features.
                        # Això es calcula per cada repetició
            ...
            if i_measure == 0:  # power
                ...
                # Agafa la mitjana del ranking de totes les repeticions d'una feature.
                # Selecciona els índex on estan les millors features.
                ...
                for i_rep in range(n_rep_RFE):
                    for ind_train, ind_test in cv_schem.split(vect_features, labels):  # false loop, just 1
                        for i_best_feat in range(1, n_feat_test_pow):
                            ...
                            perf_rk_best_feat_pow[i_band, i_rep, i_best_feat] = c_MLR.score(
                                vect_features[ind_test, :][:, ind_select], labels[ind_test])
            else:  # corr
                ...
                for i_rep in range(n_rep_RFE):
                    for ind_train, ind_test in cv_schem.split(vect_features, labels):  # false loop, just 1
                        for i_best_feat in range(1, n_feat_test_FC):
                            ...
                            # train and test for original data
                            c_MLR.fit(vect_features[ind_train, :][:, ind_select], labels[ind_train])
                            perf_rk_best_feat_FC[i_band, i_rep, i_best_feat] = c_MLR.score(
                                vect_features[ind_test, :][:, ind_select], labels[ind_test])
        ...
        for i_measure in range(n_measures):
            ...
            for i_rep1 in range(n_rep):
                for i_rep2 in range(i_rep1):
                    ...
                    stt.pearsonr(rk_FC[i_band, i_rep1, :], rk_FC[i_band, i_rep2, :])[0]
                    # shuffled surrogates
                    pearson_corr_rk_shuf[i_band, 0, i_cnt] = \
                    stt.pearsonr(np.random.permutation(rk_pow[i_band, i_rep1, :]), rk_pow[i_band, i_rep2, :])[0]
                    pearson_corr_rk_shuf[i_band, 1, i_cnt] = \
                    stt.pearsonr(np.random.permutation(rk_FC[i_band, i_rep1, :]), rk_FC[i_band, i_rep2, :])[0]
                    # increment counter
                    i_cnt += 1
    ...
    for i_band in range(n_bands):
        ...
        pp.violinplot(pearson_corr_rk_shuf[i_band, :, :].T, positions=np.arange(n_measures) + 0.2,
                      widths=[0.4] * n_measures)
        ...
        pp.figure(figsize=[4, 3])
        pp.axes([0.2, 0.2, 0.7, 0.7])
        pp.fill_between(range(n_feat_test_pow),
                        perf_rk_best_feat_pow[i_band, :, :].mean(0) - perf_rk_best_feat_pow[i_band, :, :].std(
                            0) / np.sqrt(n_rep),
                        perf_rk_best_feat_pow[i_band, :, :].mean(0) + perf_rk_best_feat_pow[i_band, :, :].std(
                            0) / np.sqrt(n_rep))
