...
res_dir = 'results/sub'+str(i_sub)+'-' + 'SP-'+type_motion[i_motion]+'-'+type_space[i_space]+ '/'
...
if i_motion == 0 or i_motion == 1:
    ...
    for i in range(2):
        start = i * 216
        ts_tmp[:, :, start:start + 108, 0] = ts_tmp2[:, :, :, i * 6 + 2]
        ts_tmp[:, :, start + 108: start + 216, 0] = ts_tmp2[:, :, :, i * 6 + 3]
        ts_tmp[:, :, start :start + 108, 1] = ts_tmp2[:, :, :, i * 6 + 4]
        ts_tmp[:, :, start + 108:start + 216, 1] = ts_tmp2[:, :, :, i * 6 + 5]
