    ...
    for i_sub, ind_sub in enumerate(subjects):
        ...
        for i_motion, type_motion in enumerate(type_motions):
            ...
            try:
                ...
                ts_tmp2 = sio.loadmat(data_dir+'dataClean-ICA3-'+str(subjects[i_sub])+'-T1.mat')['ic_data3'][:,:,:,low_ind:top_ind]

                ts_tmp = np.zeros([ts_tmp2.shape[0],ts_tmp2.shape[1],ts_tmp2.shape[2]*2,3])

                # L'última dimensió és el nombre d'estats motivacionals (Easy, Hard)
                ...
                invalid_ch1 = np.logical_or(np.abs(ts_tmp2[:,:,:,0]).max(axis=(1,2))==0, np.isnan(ts_tmp2[:,0,0,0]))
                invalid_ch2 = np.logical_or(np.abs(ts_tmp2[:,:,:,1]).max(axis=(1,2))==0, np.isnan(ts_tmp2[:,0,0,1]))
                ...
                labels = np.zeros([n_motiv,n_trials], dtype=int) # 0 = M0, 1 = M1, 2 = M2
                labels[1,:] = 1
...
sb.violinplot(data=perf_shuf_tmp2.reshape([-1,3]), linewidth=0.5, inner=None, color=[0.7,0.7,0.7])
# plot data distribution lumped for all subjects
sb.violinplot(data=perf_tmp2.reshape([-1,3]), linewidth=0.5, inner=None, palette=[[1,0.7,0.7],[0.7,1,0.7],[0.7,0.7,1]])
