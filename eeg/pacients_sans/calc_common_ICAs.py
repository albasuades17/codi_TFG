...
for variable in ['SF', 'SP']:
    ...
    for i_sub in range(n_sub):
        ...
        try: # for subject 32 (sources missing)
        ...
        if valid_sub:
            for i_motion, type_motion in enumerate(type_motions):
                try:
                    res_dir = path_dir_res+'sub'+str(subjects[i_sub])+'-'+ variable + '-' + type_motion+'-ICA/'
    ...
    n_best_src_tot = ind_best_src_sub_aligned.shape[1]

    # vector que comença a 0, acaba a n_best_src_tot i té step = n_best_src (=8)
    ...
    def sim_comp(c1, c2):
        ...
        # Mirem si els valors positius de c1 són més grans que els valors negatius.
    ...
    for i_band in range(n_bands):
        ...
        # matrices that measure the likelihood to be in the same community group
        if False:
