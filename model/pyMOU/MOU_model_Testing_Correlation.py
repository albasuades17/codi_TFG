...
#            self.Sigma = 0.5 + 0.5 * np.random.rand(self.n_nodes) * np.eye(self.n_nodes, dtype=float)
...
#        print('Lyapunov optimization, time lag:',i_tau_opt)
        ...
        while not stop_opt:
            ...
            J = -np.eye(self.n_nodes, dtype=float) / tau_x + C
    ...
    def fit_correlation(self, X, y=None, method='lyapunov', **kwargs):
        """
        Generic fit method to call the adequate specific method.
        ...
        #sos = sgn.butter(6, [0.01, 0.35], analog=False, btype='band', output='sos')
        #temp_X = sgn.sosfiltfilt(sos,centered_X, axis=0)
        
        """
        EEG pre-processing pipeline. It starts with a band-pass
        filter between 0.1 and 100Hz, a notch filter to remove power supply noise at 50Hz and its harmonics (100,
        150Hz)
        
        fs = 500 ???
        
        [low,high] = [lowfreq/fs,highfreq/fs] or [low,high] = [lowfreq/(2*fs),highfreq/(2*fs)] ???
        
        alpha=[8-12Hz] ->? [0.016, 0.024], betta=[15-32Hz] ->? [0.03, 0.064], gamma=[40-80Hz] ->? [0.08, 0.16]
        """
        #(b, a) = sgn.iirfilter(6,[0.0001, 0.5],btype='bandpass',ftype='butter')
        #temp_X = sgn.filtfilt(b,a,centered_X,axis=0)
        ...
        for i_tau in range(n_tau):
            for x in range(self.n_nodes):
                for y in range(self.n_nodes):
                    ...
                    Q_emp[i_tau,x,y] = stt.pearsonr( centered_X[0:n_T_span,x].reshape(-1), centered_X[i_tau:n_T_span+i_tau,y].reshape(-1) )[0]
        """
        """
        for x in range(self.n_nodes):
            for y in range(self.n_nodes):
                for i_tau in range(n_tau):
                    corr = sgn.correlate(centered_X[:,x], centered_X[:,y])
                    lags = sgn.correlation_lags(len(centered_X), len(centered_X))
                    corr /= np.max(corr)
                    best_lag = lags[np.argmax(corr)]
                    
                    lag = best_lag + i_tau
                    n_T_both = len(centered_X) - np.abs(lag)
                    if lag >= 0:
                        centered_X_shifted = centered_X[lag:len(centered_X),1]
                        cross_corr_0 = np.sum ( centered_X[0:n_T_both,0] * np.conj(centered_X[0:n_T_both,0]) ) / n_T_both
                        cross_corr_1 = np.sum ( centered_X_shifted * np.conj(centered_X_shifted) ) / n_T_both
                        cross_corr = ( np.sum ( centered_X[0:n_T_both,0] * np.conj(centered_X_shifted) ) / n_T_both ) / ( cross_corr_0 * cross_corr_1 )
                    else:
                        centered_X_shifted = centered_X[np.abs(lag):len(centered_X),0]
                        cross_corr_0 = np.sum ( centered_X_shifted * np.conj(centered_X_shifted) ) / n_T_both
                        cross_corr_1 = np.sum ( centered_X[0:n_T_both,1] * np.conj(centered_X[0:n_T_both,1]) ) / n_T_both
                        cross_corr = ( np.sum ( centered_X_shifted * np.conj(centered_X[0:n_T_both,1]) ) / n_T_both ) / (cross_corr_0 * cross_corr_1)**0.5
                    
                    Q_emp[i_tau,x,y] = cross_corr
        ...
        for x in range(self.n_nodes):
            for y in range(self.n_nodes):
                ...
                lags = sgn.correlation_lags(len(centered_X), len(centered_X))         
                corrXY = sgn.correlate(centered_X[:,x], centered_X[:,y])
                # lagXY = lags[np.argmax(corrXY)] This is used to get the lag between -T and T instead of 0 and 2T
                    
                for i_tau in range(n_tau):
                    
                    amplitudeXY = corrXY[np.argmax(corrXY) + i_tau]                   
                    Q_emp[i_tau,x,y] = amplitudeXY / (len(centered_X) - 1) 
        """
        ...
        for x in range(self.n_nodes):
            for y in range(self.n_nodes):
                for i_tau in range(n_tau):
                    # lags = sgn.correlation_lags(len(centered_X), len(centered_X))         
                    norm_corrXY = sgn.correlate(centered_X[:,x] / np.std(X[:,x]), 
                                                centered_X[:,y] / np.std(X[:,y])) / len(X[:,x]) 

                    # norm_corrXY /= min(len(centered_X[:,x]), len(centered_X[:,y]))
                    # lagXY = lags[np.argmax(corrXY)] This is used to get the lag between -T and T instead of 0 and 2T
                    
                    
                    amplitudeXY = norm_corrXY[np.argmax(norm_corrXY) + i_tau]               
                    Q_emp[i_tau,x,y] = amplitudeXY
    ...
    def simulate(self, T=100, dt=0.05, random_state=None):
        ...
        n_T0 = int(T0 / dt) # This is always negative
    ...
    def simulate_sameIC(self, IC ,T=100, dt=0.05, random_state=None):
        """
        Simulate the MOU process with simple Euler integration defined by the
        time step.
        ...
        noise = np.random.normal(size=[n_T, self.n_nodes], scale=(dt**0.5))
        ...
        for t in np.arange(n_T):
            ...
            x_tmp += np.dot(J_dt, x_tmp) + mu_dt + np.dot(sqrt_Sigma, noise[t])
            # Subsample timeseries (e.g. to match fMRI time resolution)
            if np.mod(t,n_sampl)==0:
                # save the result into the array
                ts[int(t/n_sampl)] = x_tmp
